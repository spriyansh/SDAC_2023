...
FROM rocker/rstudio:4 AS builder
...
RUN Rscript -e 'install.packages("bonemarrowref.SeuratData_1.0.0.tar.gz", repos = NULL, Ncpus = 24, quiet = TRUE, type = "source")'


# Start the lighter stage
FROM rocker/rstudio:4

# Create a directory to store a list of installed packages
RUN mkdir -p /installed-packages

# Copy the list of installed packages from the "builder" stage to the "lighter" stage
COPY --from=builder /installed-packages/apt-packages.txt /installed-packages/apt-packages.txt

# Install the packages from the copied list
RUN xargs -a /installed-packages/apt-packages.txt apt-get install -y && \
