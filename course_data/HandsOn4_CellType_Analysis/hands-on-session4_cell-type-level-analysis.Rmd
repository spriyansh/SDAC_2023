...
```{r,eval=T, "Load Libraries", message=FALSE}
library(Seurat)
library(SeuratData)
library(SeuratDisk)
...
```{r,eval=T, "Load Seurat Object"}
setty_seurat <- LoadH5Seurat("../HandsOn3_SingleCell_Data_QC/processed_data/setty_seurat_clustered.h5seurat")
```
...
```{r, eval=T, "Get bone marrow reference"}
...
```{r,"Azimuth Query Show",eval=T, warning = FALSE}
...
```{r, "plot predicted cell type", eval=T}
...
```{r, "select gebes from cluster 2", eval=T}
selected_cluster <- "2"
...
```{r, "combinbe", eval=T}
setty_combined <- LoadH5Seurat("../HandsOn4_CellType_Analysis/data_preparation/setty_combined_rep1-rep2.h5seurat",
                               verbose = FALSE)
...
```{r, "readRDS", eval=T}
combined_milo <- readRDS("../HandsOn4_CellType_Analysis/processed_data/combined_milo_NhoodDist.rds")
```
